...
def grid_node(comm, mscfg, pdicfg):
    ...
    #shares main visibility buffers with pdi, the rest of the data is lightweight and known before hand, so can be read directly from config files
    pdi.multi_expose("vis_share", [
        ("tmp_uvw", vis["uvw"].data, pdi.OUT),
        ("tmp_frequency", vis.coords["frequency"].data, pdi.OUT),
        ("tmp_vis", vis_view, pdi.OUT),
        ("tmp_flags", vis["flags"].data, pdi.OUT),
        ("tmp_weight", vis.visibility_acc.imaging_weight.data, pdi.OUT),
    ...
    pdi.multi_expose("psf_share", [("tmp_psf", nppsf, pdi.OUT)])

    iteration = 0

    pdi.multi_expose("resid_share", [("iteration", iteration, pdi.OUT),
            ("tmp_resid", npresid, pdi.OUT)])
